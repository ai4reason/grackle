...
class PremiseRunner(Runner):
   ...
   def name(self, params):
      ...
      conf = self.conf_prefix+hashlib.sha224(args.encode()).hexdigest()
      open(path.join(self.conf_dir,conf),"w").write(args)
      return conf

   def recall(self, conf):
      args = open(path.join(self.conf_dir,conf)).read().strip()
